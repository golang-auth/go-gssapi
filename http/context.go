...
// Record the http.Server in the connection's context which is passed to Handlers
// as part of the http.Request context.
func stashServerContext(ctx context.Context, s *http.Server) context.Context {
...
func getServerContext(ctx context.Context) *http.Server {
	server, ok := ctx.Value(serverContextKey).(*http.Server)
	if !ok {
		return nil
	}
	return server
