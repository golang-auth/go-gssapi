...
func TestStashAndGetServerContext(t *testing.T) {
	baseCtx := context.Background()
	server := &http.Server{}

	ctxWithServer := stashServerContext(baseCtx, server)

	got := getServerContext(ctxWithServer)
	if got == nil {
		t.Fatalf("expected non-nil server from context")
	}
	if got != server {
		t.Fatalf("expected server %p, got %p", server, got)
	}

	// Unknown type or missing key should return nil
	if getServerContext(baseCtx) != nil {
		t.Fatalf("expected nil server from base context")
	}
}
